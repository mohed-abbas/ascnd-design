...
**Decision 2 (2026-07-02, implemented):** the low-tier fallback is **clear glass, not frost** — `blur(22px)` over the bright reel on the blue sky reads as a milky blue bar (stakeholder-rejected). The fallback is a transparent body + crisp rim with **no `backdrop-filter` at all**: the framed phrase stays sharp (which is what the design frames anyway) and the low-tier pill costs zero per frame — strictly better than frost for weak machines. The Safari/Firefox engine fallback keeps its deliberate frost (`724629c`) — that's a different constraint (broken displacement, capable GPU), revisit separately if desired.

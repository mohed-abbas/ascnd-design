...
**Decision (2026-07-02, implemented):** the gate is **latched, not live** — a reactive gate would let a mid-pin watchdog step-down swap liquid glass → frost in front of the user (rejected by the stakeholder as a visible material downgrade). The gate follows the store only while the pill is far off-screen and freezes permanently as the section approaches the viewport: weak machines show frost from the pill's very first appearance; everyone else keeps the displacement for the whole session, even if the watchdog later demotes the rest of the page.

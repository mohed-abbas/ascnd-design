/**
 * Gallery — 1:1 port of `codrops-depth-gallery-main/src/Experience/Gallery.js`.
 *
 * Owns the depth-stacked image planes: layout, opacity crossfade between the two
 * planes the camera is between, pointer parallax, velocity-driven "breath"
 * (tilt + scale pulse), and the mood/blend colour sampling the Background reads.
 * Ported verbatim; the only changes: the tweakpane debug bindings are removed,
 * and the pointer listeners bind to the section's canvas element instead of
 * `window` (so parallax responds only while the pointer is over the section).
 */
import * as THREE from "three";
import { galleryPlaneData, type GalleryPlaneDatum } from "../portfolio-data";
import type { MoodColors } from "./background";
...
class Gallery {
  ...
  planes: PlaneMesh[];
  texturesBySource: Map<string, THREE.Texture>;
  useTextures: boolean;
  planeGap: number;
  desktopPlaneScale: number;
  mobilePlaneScale: number;
  ...
  parallaxAmountX: number;
  parallaxAmountY: number;
  parallaxSmoothing: number;
  pointerTarget: THREE.Vector2;
  pointerCurrent: THREE.Vector2;

  breathEnabled: boolean;
  breathTiltAmount: number;
  breathScaleAmount: number;
  breathSmoothing: number;
  breathGain: number;
  breathIntensity: number;
  targetBreathIntensity: number;

  gestureParallaxEnabled: boolean;
  gestureParallaxAmountY: number;
  gestureParallaxSmoothing: number;
  driftCurrent: number;
  driftTarget: number;

  private pointerTargetElement: HTMLElement | null;
  ...
  constructor() {
    ...
    // Pointer events (bound to the section canvas via bindPointerEvents)
    this.pointerTargetElement = null;
  ...
  setPlanes(scene: THREE.Scene) {
    ...
    this.planeConfig.forEach((plane, index) => {
      ...
      const planeMesh = new THREE.Mesh(planeGeometry, planeMaterial) as PlaneMesh;
  ...
  bindPointerEvents(target: HTMLElement) {
    this.pointerTargetElement = target;
  ...
  updatePlaneMotion(scroll: ScrollLike | null = null) {
  ...
  update(camera: THREE.PerspectiveCamera | null = null, scroll: ScrollLike | null = null) {
    if (!camera) return;
  ...
  dispose() {
    if (this.pointerTargetElement) {
      this.pointerTargetElement.removeEventListener("pointermove", this.onPointerMove);
      this.pointerTargetElement.removeEventListener("pointerleave", this.onPointerLeave);
      this.pointerTargetElement = null;
    }
  }
}

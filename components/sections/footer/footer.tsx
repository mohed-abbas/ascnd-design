...
export default function Footer() {
  return (
    ...
    >
      ...
      {/* Content column (747:489) — centre-anchored in the band on desktop; on
          mobile it drops to static, gutter-padded flow. Fixed 1197px wide,
          capped so it never overflows a narrow viewport (the wordmark scales
          with it). */}
      <div className="absolute left-1/2 top-1/2 flex w-[1197px] max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-[43px] max-md:static max-md:translate-x-0 max-md:translate-y-0 max-md:mx-auto max-md:gap-[40px]">
      ...
      {/* Rocks (747:510 left / 747:511 right) — rendered AFTER the content so
          they PAINT ON TOP of the wordmark: the giant "ascnd"'s ends tuck behind
          the cliffs, and as the logo rises (FooterReveal) it emerges from behind
          them (the immersion effect). Each is pixel-mapped to its Figma box and
          pinned to a bottom frame edge; the 825×1024 source cutout is STRETCHED
          to the box (Figma fill scale-mode — the boxes' aspect ratios differ from
          the source on purpose). pointer-events-none so they never intercept the
          nav above; dropped below md like the hero cliffs. */}

/**
 * GLSL for the cursor fluid-trail, ported verbatim from
 * `cursor-trail-main/src/shaders/trail/{vertex,fragment}.glsl`.
 *
 * Inlined as string constants rather than imported as `.glsl`: the source
 * project used Vite's `vite-plugin-glsl`, which Turbopack (Next 16) doesn't
 * have, and inlining avoids configuring a raw loader.
 *
 * The only edit from the original is the three injected blob colours
 * (search "BRAND PALETTE" below), retuned from the source's purple/teal/white
 * to an on-brand luminous-blue that reads as an additive extension of the
 * #62abff sky. See docs/cursor-trail.md.
 */

// Full-screen triangle passthrough — writes clip-space positions directly, so
// no camera/projection is involved.
...
// The trail itself: samples its own previous frame (uMap), advects it along a
// curl-noise flow field, fades it, and injects fresh colour at the pointer.
